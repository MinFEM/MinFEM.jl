...
"""
    restrict_multivector(x::AbstractVector{Float64}, m::Int64, qdim::Int64; block::Int64=1) -> Vector{Float64}
...
function restrict_multivector(x::AbstractVector{Float64}, m::Int64, qdim::Int64; block::Int64=1)
    ...
    elseif block == 1
        ...
        for i = 1:m
            for j = 1:qdim
                v[block*(i-1) + j] = sum(x[(qdim*block*(i-1)+(j-1)*block+1):(qdim*block*(i-1)+block*j)])
            end
        end
        return v
    end
...
"""
    prolong_multivector(x::AbstractVector{Float64}, m::Int64, qdim::Int64; block::Int64=1) -> Vector{Float64}
...
function prolong_multivector(x::AbstractVector{Float64}, m::Int64, qdim::Int64; block::Int64=1)
    ...
    elseif block == 1
        ...
        for i = 1:m
            for j = 1:qdim
                v[(qdim*block*(i-1)+(j-1)*block+1):(qdim*block*(i-1)+block*j)] = x[(block*(i-1)+1):(block*i)]
            end
        end
        return v
    end
...
"""
    assemble_weightmultivector(mesh::Mesh; qdim::Int64=1, order::Int64=1) -> Vector{Float64}
...
"""
    assemble_weightmultivector_boundary(mesh::Mesh; qdim::Int64=1, order::Int64=1) -> Vector{Float64}
...
"""
    assemble_derivativematrix(mesh::Mesh; qdim=1) -> SparseMatrixCSC{Float64, Int64}
    
Returns the discrete derivative matrix for all elements of mesh and image dimension qdim.
"""
function assemble_derivativematrix(mesh::Mesh; qdim=1)   
...
"""
    assemble_laplacian(mesh::Mesh; qdim=1) -> SparseMatrixCSC{Float64, Int64}
    assemble_laplacian(D::SparseMatrixCSC{Float64, Int64}, w::AbstractVector{Float64}) -> SparseMatrixCSC{Float64, Int64}
...
"""
function assemble_laplacian(mesh::Mesh; qdim=1)
...
"""
    assemble_derivativematrix_boundary(mesh::Mesh; boundaryElements::Set{Int64}=Set{Int64}(), qdim=1) -> SparseMatrixCSC{Float64, Int64}
    
Returns the discrete derivative matrix for all or specied boundary elements of mesh and image dimension qdim.
...
"""
function assemble_derivativematrix_boundary(mesh::Mesh; boundaryElements::Set{Int64}=Set{Int64}(), qdim=1)
...
"""
    assemble_normalderivativematrix(mesh::Mesh; boundaryElements::Set{Int64}=Set{Int64}(), qdim=1) -> SparseMatrixCSC{Float64, Int64}
    
Returns the discrete normal derivative matrix for all or specied boundary elements of mesh and image dimension qdim.
Workaround based on the derivate tensor for the corresponding full element
and the fact that the gradient ist constant on the element. 
"""
function assemble_normalderivativematrix(mesh::Mesh; boundaryElements::Set{Int64}=Set{Int64}(), qdim=1)
    if isempty(boundaryElements)
        boundaryElements = Set{Int64}(1 : mesh.nboundelems)
    end
...
"""
    assemble_basismatrix(mesh::Mesh; qdim=1) -> SparseMatrixCSC{Float64, Int64}
...
"""
    assemble_massmatrix(mesh::Mesh; qdim::Int64=1, order::Int64=1) -> SparseMatrixCSC{Float64, Int64}
    assemble_massmatrix(E::SparseMatrixCSC{Float64, Int64}, w::AbstractVector{Float64}) -> SparseMatrixCSC{Float64, Int64}

Returns the mass matrix with given local integration order for all elements of mesh and image dimension qdim.
Can either be assembled directly or from derivative matrix and weight vector.
"""
...
function assemble_massmatrix(E::SparseMatrixCSC{Float64, Int64}, w::AbstractVector{Float64})
    M = E' * Diagonal(w) * E
    dropzeros!(M)
    return M
end
...
"""
    assemble_basismatrix_boundary(mesh::Mesh; boundaryElements::Set{Int64}=Set{Int64}(), qdim::Int64=1, order::Int64=1) -> SparseMatrixCSC{Float64, Int64}
...
"""
function assemble_basismatrix_boundary(mesh::Mesh; boundaryElements::Set{Int64}=Set{Int64}(), qdim::Int64=1, order::Int64=3)
    if isempty(boundaryElements)
        boundaryElements = Set{Int64}(1 : mesh.nboundelems)
    end
...
"""
    assemble_massmatrix_boundary(mesh::Mesh; boundaryElements::Set{Int64}=Set{Int64}(), qdim::Int64=1, order::Int64=1) -> SparseMatrixCSC{Float64, Int64}
    assemble_massmatrix_boundary(E::SparseMatrixCSC{Float64, Int64}, w::AbstractVector{Float64}) -> SparseMatrixCSC{Float64, Int64}

Retruns the mass matrix with given local integration order for all or specified boundary elements of mesh and image dimension qdim.
Can either be assembled directly or from derivative matrix and weight vector.
"""
function assemble_massmatrix_boundary(mesh::Mesh; boundaryElements::Set{Int64}=Set{Int64}(), qdim::Int64=1, order::Int64=3)
    if isempty(boundaryElements)
        boundaryElements = Set{Int64}(1 : mesh.nboundelems)
    end
...
function assemble_massmatrix_boundary(E::SparseMatrixCSC{Float64, Int64}, w::AbstractVector{Float64})
    M = E' * Diagonal(w) * E
    dropzeros!(M)
    return M
end
...
"""
    assemble_cubicterm(mesh::Mesh, y::AbstractVector)
...
"""
    assemble_cubicderivativematrix(mesh::Mesh, y::AbstractVector)
...
"""
    assemble_cubicsecondderivativematrix(mesh::Mesh, y::AbstractVector, p::AbstractVector)

Returns the second derivative of the cubic term of the standard semilinear elliptic equation 
around the state y.
"""
function assemble_cubicsecondderivativematrix(mesh::Mesh, y::AbstractVector, p::AbstractVector; order::Int64=3)
...
"""
    assemble_dirichletcondition!(A, DI::Set{Int64}; rhs=[], bc=[], qdim=1, insert=1.0)
    assemble_dirichletcondition!(A, DI::Set{Boundary}; rhs=[], bc=[], qdim=1, insert=1.0)
...
"""
function assemble_dirichletcondition!(A, DI::Set{Int64}; rhs=[], bc=[], qdim=1, insert=1.0)
    if rhs != [] && bc != []
        for i in DI
            for d = 1:qdim
                ii = qdim * (i-1) + d
...
function assemble_dirichletcondition!(A, DI::Set{Boundary}; rhs=[], bc=[], qdim=1, insert=1.0)
    assemble_dirichletcondition!(A, extract_nodes(DI); rhs=rhs, bc=bc, qdim=qdim, insert=insert)
end

"""
    assemble_dirichletprojection(nnodes::Int64, DI::Set{Int64};qdim=1) -> SparseMatrixCSC{Float64, Int64}

Returns the projection matrix onto the Dirichlet nodes where the input ncoeffs is understood as qdim*nnodes.
"""
function assemble_dirichletprojection(ncoeffs::Int64, DI::Set{Int64}; qdim=1)
    bcids = Set{Int64}()
    for r = 1:qdim
