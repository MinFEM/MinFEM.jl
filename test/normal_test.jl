...
function test_normals()
    ...
    for bel in extract_elements(select_boundary(mesh, 1001))
        soln[2*(bel-1)+1 : 2*bel] = [0,-1]
    end
    for bel in extract_elements(select_boundary(mesh, 1002))
        soln[2*(bel-1)+1 : 2*bel] = [0,1]
    end
    for bel in extract_elements(select_boundary(mesh, 1003))
        soln[2*(bel-1)+1 : 2*bel] = [-1,0]
        solDnf[bel] = -0.5
    end
    for bel in extract_elements(select_boundary(mesh, 1004))
        soln[2*(bel-1)+1 : 2*bel] = [1,0]
        solDnf[bel] = 0.5
    end
    ...
    for bel in extract_elements(select_boundary(mesh, 1005))
        soln[3*(bel-1)+1 : 3*bel] = [0,-1,0]
        solDng[3*(bel-1)+1 : 3*bel] = [0,0,-0.5]
    end
    for bel in extract_elements(select_boundary(mesh, 1006))
        soln[3*(bel-1)+1 : 3*bel] = [0,1,0]
        solDng[3*(bel-1)+1 : 3*bel] = [0,0,0.5]
    end
